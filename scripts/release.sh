...
# Gobbl release: build → sign → notarize → staple → DMG → Sparkle-sign → appcast → cask → (upload)
#
# Usage:
#   scripts/release.sh <version> [--notes notes.md] [--upload] [--github] [--skip-notarize]
#
# Examples:
#   scripts/release.sh 0.1.0                      # build + notarize locally, nothing published
#   scripts/release.sh 0.1.0 --notes release-notes/0.1.0.md --upload --github
#
# Output: build/release/Gobbl-<version>.dmg, appcast.xml and gobbl.rb (Homebrew cask)
#
...
#     Keychain account "gobbl"                          local default
...
grep -q 'SUPublicEDKey: "[A-Za-z0-9+/=]\{40,\}"' project.yml || die "SUPublicEDKey not set in project.yml"
...
# The MediaRemote adapter is loaded by /usr/bin/perl, not by us, but notarization
# still requires every Mach-O in the bundle to carry our Developer ID signature.
sign "$APP_PATH/Contents/Resources/MediaRemoteAdapter.framework"
...
if [ -n "${SPARKLE_KEY_FILE:-}" ]; then
  ...
  ED=$("$SIGN_UPDATE" --account gobbl "$DMG")
fi
...
for line in open(sys.argv[1]).read().splitlines():
    ...
    if s.startswith(("- ", "* ")):
        ...
        if s and not s.startswith("#"): out.append("<p>%s</p>" % html.escape(s))
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    ...
    <description>Gobbl updates</description>
    <language>en</language>
    <item>
      <title>Gobbl ${VERSION}</title>
...
RUBY
echo "  $OUT/gobbl.rb (sha256 ${SHA:0:12}…) — copy into the xeveio/homebrew-tap repo"

# ---------------------------------------------------------------------------
if $UPLOAD; then
  step "Upload to ${SPACE}/${SPACE_PREFIX}/ (served at ${PUBLIC_BASE}/)"
  # Locally, a bucket-scoped key lives in ~/.secrets (readwrite on xeve-downloads only). CI passes env vars.
  for f in "$HOME/.secrets/do-spaces-gobbl.env" "$HOME/.secrets/do-spaces-tidybug.env"; do
    if [ -z "${DO_SPACES_KEY:-}" ] && [ -f "$f" ]; then set -a; . "$f"; set +a; fi
  done
